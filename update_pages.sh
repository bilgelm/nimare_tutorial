...
find . -type f -name "*.ipynb" \
    -exec gsed -i 's/execution_count\b.*$/execution_count": null,/' {} +
...
find docs -type f -name "*.html" -exec gsed -i 's/ipynb\&/html\&/g' {} +
find docs -type f -name "*.html" -exec gsed -i 's/ipynb#/html#/g' {} +
find docs -type f -name "*.html" -exec gsed -i 's/ipynb\"/html\"/g' {} +

# Delete the Button "Show HTML code" from index.html
gsed -i '/Show HTML code/d' docs/index.html

# Add Google Analytics script to each homepage
for h in `find docs -maxdepth 2 -name "*html"`
do
    gsed '/<\/head>/ {r static/template_google_analytics.rst
    d}' $h > tmp.rst
